...
const CheckoutForm = () => {
  ...
  const {sendSaleRequest} = useCloverContext()

    useEffect(() => {
        makeSaleRequest()
    }, [])

  const makeSaleRequest = () => {      //  returns transaction request for sale
      ...
      request.setAmount(total_amount)
      sendSaleRequest(request)
  }
  return (
      <div>
          <article>
          ...
          <p>Please use the Clover POS to make payment.</p>
          </article>
      </div>
  )
}
